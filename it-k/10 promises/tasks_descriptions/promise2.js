...
//2. Верно ли утверждение: что в 8 строке как только callback внутри pr.then() будет выполнен то промис в константе pr2 сменится с pending на fulfilled?
// (а до этого там будет promise со статусом pending.)

//3. Измените код так что бы в режиме дебага. Можно было бы посмотреть результат fetch в 8 строке
const pr = fetch("https://google.com");

const pr2 = pr.then((data) => console.log("pr then"));
